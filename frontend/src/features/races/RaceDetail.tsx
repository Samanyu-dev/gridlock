...
import type { RaceFull } from '../../lib/types'
      ...
      <div className="container">
        ...
        <div className="panel" style={{ overflow: 'hidden', marginBottom: 20 }}>
          <div className="row between wrap" style={{ padding: 28 }}>
            <div>
              <span className="eyebrow">Round {r.round}{r.is_sprint ? ' · Sprint weekend' : ''}</span>
          ...
          <div className="panel" style={{ overflow: 'hidden' }}>
            {r.sessions.map((s, i) => (
              <div key={s.kind} className="row between" style={{ padding: '14px 20px', borderBottom: i < r.sessions.length - 1 ? '1px solid var(--line-soft)' : 'none', background: s.kind === 'RACE' ? 'var(--surface-2)' : 'transparent' }}>
                <span className="row gap-2"><span className="chip" style={{ minWidth: 60, justifyContent: 'center', color: s.kind === 'RACE' ? 'var(--red)' : undefined }}>{s.kind}</span><span style={{ fontWeight: 600 }}>{s.label}</span></span>
        ...
        {tab === 'Results' && (
          <div className="panel" style={{ overflow: 'hidden' }}>
            {done ? (
              ...
              <div className="empty"><p className="text-dim">Results drop after the chequered flag.</p></div>
            )}
          </div>
        )}
        ...
        {tab === 'Grid' && (
          <div className="panel" style={{ overflow: 'hidden' }}>
            ...
            ) : <div className="empty"><p className="text-dim">Qualifying grid appears after Saturday.</p></div>}
          </div>
        )}
      </div>
    </div>
